...
// Initialize hover states for cursor
function initCursorHoverListeners() {
  const hoverables = document.querySelectorAll('a, button, input, textarea, .glass-card, .tech-pill, #menu-toggle');
...
// Projects Data (Configured with optional image parameter, default CSS placeholder)
const projects = [
  { img: '', title: 'Nimbus Analytics', desc: 'A real-time analytics dashboard with custom data viz components.', tech: ['HTML','Tailwind','JavaScript','Chart.js'] },
  { img: '', title: 'Aurora Commerce', desc: 'Headless e-commerce front end with motion-rich product showcases.', tech: ['HTML','Tailwind','JavaScript'] },
  { img: '', title: 'Helix Studio', desc: 'Editorial portfolio for a creative studio — bold type, smooth scroll.', tech: ['HTML','CSS','JavaScript'] },
  { img: '', title: 'Tide Banking', desc: 'Mobile-first marketing site for a modern neobank.', tech: ['HTML','Tailwind','Bootstrap'] },
];
...
function renderProjects(lang) {
  ...
  grid.innerHTML = projects.map((p, i) => {
    ...
    const hasImage = p.img && p.img !== '';
    const imageHtml = hasImage
      ? `<img src="${p.img}" alt="${title}" loading="lazy" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />`
      : `<div class="absolute inset-0 bg-gradient-to-br from-cyan/15 via-blue/5 to-transparent opacity-60 group-hover:opacity-100 transition-opacity duration-500"></div>
         ...
         <div class="relative font-display text-4xl font-black tracking-wider opacity-10 group-hover:opacity-20 transition-opacity duration-500 select-none uppercase" style="color:var(--cyan)">
           ${title.split(' ').map(w => w[0]).join('')}
         </div>`;
    return `
      <article class="glass-card project-card reveal group">
        <div class="aspect-[16/10] overflow-hidden border-b flex items-center justify-center relative" style="border-color:var(--border-color);background:linear-gradient(135deg,rgba(29,78,216,0.08),rgba(34,211,238,0.04))">
          ${imageHtml}
        </div>
        <div class="p-7">
          <h3 class="font-display text-xl font-semibold group-hover:text-cyan transition-colors duration-300">${title}</h3>
          <p class="mt-2 text-sm leading-relaxed">${desc}</p>
          <div class="mt-5 flex flex-wrap gap-2">
            ${p.tech.map(tech => `<span class="text-xs px-2.5 py-1 rounded-full" style="background:var(--bg-pill);border:1px solid var(--border-color);color:var(--text-secondary)">${tech}</span>`).join('')}
          </div>
          <div class="mt-6 flex gap-3">
            <a href="#" class="text-sm px-4 py-2 rounded-full bg-gradient-to-r from-cyan to-blue font-medium hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] transition" style="color:#04060d">${t['project-live']}</a>
            <a href="#" class="text-sm px-4 py-2 rounded-full hover:opacity-80 transition" style="background:var(--bg-pill);border:1px solid var(--border-color);color:var(--text-secondary)">${t['project-github']}</a>
          </div>
        </div>
      </article>
    `;
...
const team = [
  { name: 'Omar Tantawy', initials: 'OT', img: '', phone: '201061720405', linkedin: 'https://linkedin.com/in/omar-tantawy-a74a96376' },
  { name: 'Abdelrahman Mamdouh', initials: 'AM', img: '', phone: '201000215337', linkedin: 'https://www.linkedin.com/in/abdelrhman-mamdoh-a80b2b33b' },
  { name: 'Mohamed Waled', initials: 'MW', img: '', phone: '201221542589', linkedin: 'https://www.linkedin.com/in/mohamed-waled-elsamak/' },
  { name: 'Khaled Amr', initials: 'KA', img: '', phone: '201102740002', linkedin: 'https://linkedin.com/in/khᥲᥣᥱd-amr-263334343/' },
  { name: 'Abdelrahman Ahmed', initials: 'AA', img: '', phone: '201279109111', linkedin: 'https://www.linkedin.com/in/abdelrahman-ahmed-78b9a3358/' },
  { name: 'Belal Yousry', initials: 'BY', img: '', phone: '201224160430', linkedin: 'https://www.linkedin.com/in/belal-deyab-853659392' }
];
...
function renderTestimonials(lang) {
  ...
  grid.innerHTML = items.map((item, i) => `
    <div class="glass-card p-8 reveal group" style="transition-delay:${i*70}ms">
      <div class="text-cyan text-3xl font-display leading-none group-hover:text-blue transition-colors duration-300">"</div>
      <p class="mt-3 leading-relaxed text-[15px]">${item.quote}</p>
      <div class="mt-7 pt-5" style="border-top:1px solid var(--border-section)">
        <div class="font-semibold text-sm group-hover:text-cyan transition-colors duration-300">${item.name}</div>
        <div class="text-xs mt-0.5" style="color:var(--text-muted)">${item.role}</div>
      </div>
    </div>
  `).join('');
  grid.querySelectorAll('.reveal').forEach(el => io.observe(el));
...
const translations = {
  en: {
    ...
    'hero-desc': 'Wave is a front-end team designing and shipping modern, lightning-fast websites and product UIs for forward-thinking brands. Pixel-perfect, performance‑obsessed, future‑ready.',
    ...
    'about-body': 'Wave is a front-end development team focused on building digital experiences that feel as good as they look. We partner with founders, designers and product teams to ship websites and interfaces that load fast, scale gracefully, and feel inevitable.',
    ...
    'services-body': 'From single-page launches to full product surfaces — we handle the front end end to end.',
    ...
    // Testimonials
    'section-testimonials-tag': '06 — Testimonials',
    'section-testimonials-title': 'Trusted by builders.',

    // Contact
    'section-contact-tag': '07 — Contact',
    ...
    'service-1-desc': 'High-converting one-pagers built for launches, campaigns and product reveals.',
    ...
    'service-3-desc': 'Distinctive portfolios for studios, creators and freelancers that stand out.',
    ...
    'testimonial-1-quote': 'Wave shipped our marketing site in three weeks and it outperformed every benchmark we set.',
    'testimonial-1-name': 'Mira Chen',
    'testimonial-1-role': 'CEO, Northwind',
    'testimonial-2-quote': 'The most detail-obsessed front-end team we\'ve worked with. The interface just feels right.',
    'testimonial-2-name': 'David Park',
    'testimonial-2-role': 'Head of Product, Lumen',
    'testimonial-3-quote': 'Pixel-perfect, fast, and a pleasure to collaborate with. We hired them again the same quarter.',
    'testimonial-3-name': 'Elena Rossi',
    'testimonial-3-role': 'Founder, Atlas Studio',
  },
  ...
  ar: {
    ...
    'hero-desc': 'Wave فريق متخصص في تصميم وتطوير مواقع حديثة وسريعة وواجهات منتجات احترافية للعلامات التجارية الطموحة. دقة بكسل، أداء لا يُضاهى، جاهز للمستقبل.',
    ...
    'about-body': 'Wave فريق تطوير واجهات أمامية متخصص في بناء تجارب رقمية تبدو رائعة بقدر ما تعمل بكفاءة. نتعاون مع المؤسسين والمصممين وفرق المنتجات لتسليم مواقع وواجهات تُحمَّل بسرعة وتتوسع بسلاسة.',
    ...
    'services-body': 'من الصفحات الواحدة إلى أسطح المنتجات الكاملة — نتولى الواجهة الأمامية من الألف إلى الياء.',
    ...
    // Testimonials
    'section-testimonials-tag': '06 — آراء العملاء',
    'section-testimonials-title': 'يثق بنا البُناة.',
    ...
    'testimonial-2-quote': 'أكثر فريق واجهة أمامية اهتماماً بالتفاصيل عملنا معه. الواجهة تبدو صحيحة تماماً.',
